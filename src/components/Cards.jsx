...
export const ServiceCard = ({ icon: Icon, title, description, link }) => (
    <Link to={link || "/services/details"} className="group p-6 sm:p-8 rounded-2xl bg-[#0a111a]/60 backdrop-blur-xl border border-cyber-700/50 hover:border-accent-cyan/50 transition-all duration-500 box-glow-hover flex flex-col h-full hover:-translate-y-2 relative overflow-hidden">
        ...
        <div className="relative z-10 w-14 h-14 rounded-xl flex items-center justify-center mb-6 bg-cyber-900/80 border border-cyber-700 group-hover:bg-accent-blue/10 group-hover:border-accent-cyan/50 transition-all duration-300">
            <Icon className="w-7 h-7 text-accent-blue group-hover:text-accent-cyan group-hover:drop-shadow-[0_0_8px_rgba(57,255,136,0.8)] transition-all" />
        </div>
        <h3 className="relative z-10 text-xl font-bold text-white mb-3 group-hover:text-accent-cyan transition-colors">{title}</h3>
        <p className="relative z-10 text-slate-400 mb-6 leading-relaxed flex-grow group-hover:text-slate-300 transition-colors text-sm sm:text-base lg:text-lg">{description}</p>
        <span className="relative z-10 inline-flex items-center text-accent-cyan font-semibold group-hover:text-white transition-colors mt-auto w-max">
            Learn More
...
export const FeatureCard = ({ icon: Icon, title, description }) => (
    <div className="p-6 sm:p-8 rounded-2xl bg-cyber-900/40 backdrop-blur-xl border border-cyber-800/50 hover:border-cyber-700 hover:bg-cyber-800/60 transition-colors duration-300 relative overflow-hidden group flex flex-col h-full">
        ...
        <div className="relative z-10 w-14 h-14 rounded-full flex items-center justify-center mb-6 bg-[#05080c] text-accent-cyan border border-accent-cyan/20 group-hover:border-accent-cyan/50 shadow-[0_0_15px_rgba(57,255,136,0.1)] group-hover:shadow-[0_0_20px_rgba(57,255,136,0.3)] transition-all">
            <Icon className="w-6 h-6" />
        </div>
        <h3 className="relative z-10 text-xl font-bold text-white mb-3">{title}</h3>
        <p className="relative z-10 text-slate-400 leading-relaxed flex-grow text-sm sm:text-base lg:text-lg">{description}</p>
    </div>
);
...
export const CaseStudyCard = ({ image, title, description, category, link }) => (
    <Link to={link || "/case-studies/details"} className="group block relative rounded-2xl overflow-hidden shadow-lg min-h-[300px] w-full sm:min-h-[400px] w-full flex flex-col justify-end border border-cyber-700 hover:border-accent-blue transition-all duration-300 box-glow-hover">
        ...
        />
        <div className="relative z-20 p-6 sm:p-8 transform translate-y-0 lg:translate-y-4 lg:group-hover:translate-y-0 transition-transform duration-300">
...
export const PricingCard = ({ plan, price, description, isPopular, features }) => (
    <div className={`p-6 sm:p-8 rounded-3xl border ${isPopular ? 'border-accent-blue bg-cyber-800/80 shadow-[0_0_30px_rgba(57, 255, 136,0.15)] scale-105 z-10' : 'border-cyber-700 bg-cyber-900'} relative backdrop-blur-sm flex flex-col h-full`}>
        {isPopular && (
        ...
        <div className="flex items-baseline mb-6">
            ...
            <span className="text-slate-400 ml-2 text-sm sm:text-base lg:text-lg">/month</span>
        </div>
        <p className="text-slate-400 mb-8 border-b border-cyber-700 pb-6 text-sm sm:text-base lg:text-lg">{description}</p>
        <ul className="space-y-4 mb-8 flex-grow">
            {features.map((feature, idx) => (
                <li key={idx} className="flex items-start text-slate-300 text-sm sm:text-base lg:text-lg">
