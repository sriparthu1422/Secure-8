            ...
            <section className="py-12 sm:py-20 lg:py-28 bg-cyber-950 relative border-t border-cyber-800/50">
                ...
                <LayoutContainer className="relative z-10">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 sm:gap-6 lg:gap-8 mt-10">
                        <ServiceCard icon={FiHash} title="Information Security" description={<><strong className="block mb-2 text-slate-200">Build a strong security foundation.</strong>Learn how organizations protect systems, networks, and data using industry best practices.</>} link="/academics/info-security" />
                        <ServiceCard icon={FaBug} title="Penetration Testing" description={<><strong className="block mb-2 text-slate-200">Find vulnerabilities before attackers do.</strong>Understand the full VAPT lifecycle through real lab environments and live simulations.</>} link="/academics/vapt" />
                        <ServiceCard icon={FiCode} title="API Security Testing" description={<><strong className="block mb-2 text-slate-200">Secure modern interfaces.</strong>Detect flaws in machine-to-machine communications. Master IDOR, BOLA, and other top API risks.</>} link="/academics/api-security" />
                        <ServiceCard icon={FiCloud} title="Cloud Security" description={<><strong className="block mb-2 text-slate-200">Protect the modern stack.</strong>Secure AWS, Azure, and GCP environments from misconfigurations using practical training.</>} link="/academics/cloud-security" />
                        <ServiceCard icon={FaFingerprint} title="Identity & Access (IAM)" description={<><strong className="block mb-2 text-slate-200">The first line of defense.</strong>Gain hands-on experience with tools like Okta and SailPoint while learning real enterprise use cases.</>} link="/academics/iam" />
                        <ServiceCard icon={FiBook} title="Career Accelerator" description={<><strong className="block mb-2 text-slate-200">Job-Ready Training.</strong>Resume building, mock interviews, and placement guidance to help you start your career.</>} link="/academics/career-prep" />
                        <ServiceCard icon={FiSmartphone} title="AppSec (Web & Mobile)" description={<><strong className="block mb-2 text-slate-200">Secure the experience.</strong>Learn to identify vulnerabilities like XSS and CSRF in web and mobile applications.</>} link="/academics/mobile-security" />
                    </div>
                </LayoutContainer>
            </section>
